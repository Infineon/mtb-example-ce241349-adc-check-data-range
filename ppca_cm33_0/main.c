...
* variables used in the code. Also, it reads the ADC read data and DCMP status in
* periodic intervals and copies the date to the shared memory for consuming by
* the main CPU.
*
* Parameters:
*  void
...
int main(void)
{
     ...
     for(;;)
     {
          ...
          *adc_data = Cy_PPCA_ADC_Read_ADC_Data(ADC_HW, 0);
          ...
          /* Reading DCMP status */
          *dcmp_state = Cy_PPCA_DCMP_Get_Status(DCMP_HW);
