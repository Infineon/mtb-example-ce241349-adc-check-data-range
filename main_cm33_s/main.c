...
const cy_stc_ppca_cnfg_ppcaout_input_selector_t ppca_button_input = {
    .inputSelSrc = 14,
...
int main(void)
{
    ...
    uint32_t *adc_data   = (uint32_t *)PPCA_CPU0_M4_VAR_ADDRESS;
    uint32_t *dcmp_state = adc_data + 1;
    *adc_data            = 0;
    *dcmp_state          = 0;
    ...
    printf("\x1b[2J\x1b[;H");
    printf("************************************************************\r\n");
    printf("PSOC Control C3M/P8: ADC DCMP range check\r\n");
    printf("************************************************************\r\n\n");
    ...
    /* Configuring PPCA input selection */
    Cy_PPCA_CNFG_PPCA_Input_Selector(PPCA_CNFGCNFG_HW, &ppca_button_input, ppca_button_input.inputSelSrc);
    ...
    /* Configuring EPU processing unit to receive trigger signal from ATOP DCMP. */
    Cy_PPCA_EPU_PU_T1_Configure(DCMP_TRIG_HW, DCMP_TRIG_INDEX, &DCMP_TRIG_put1_config);
    Cy_PPCA_EPU_PU_T1_Enable(DCMP_TRIG_HW, DCMP_TRIG_INDEX, DCMP_TRIG_ENABLE_MODE);

    /* Configuring EPU processing unit to receive press signal from GPIO connected to a button. */
    Cy_PPCA_EPU_PU_T2_Configure(SW_INPUT_HW, SW_INPUT_INDEX, &SW_INPUT_put2_config);
    Cy_PPCA_EPU_PU_T2_Enable(SW_INPUT_HW, SW_INPUT_INDEX, SW_INPUT_ENABLE_MODE);
    ...
    /* Configuring EPU combiner to route the signal from DCMP to PWM to kill PWM*/
    Cy_PPCA_EPU_Combo_Configure(PWM_KILL_HW, PWM_KILL_INDEX, &PWM_KILL_combo_config);

    /* Configuring EPU combiner to route the signal from GPIO to PWM to start a PWM again*/
    Cy_PPCA_EPU_Combo_Configure(PWM_START_HW, PWM_START_INDEX, &PWM_START_combo_config);
    ...
    Cy_PPCA_DCMP_Init(DCMP_HW, &DCMP_config);

    /* Enabling ATOP Digital Comparator */
    Cy_PPCA_DCMP_Enable(DCMP_HW);
    ...
    for (;;)
    {
        /* Checking for DCMP state whether the ADC
         * value crossed the threshold or not. */
        if(1 == *dcmp_state)
        {
            printf("\r\n ADC Data: %ld, Above set DCMP threshold 2047. PWM Killed.", (long int)*adc_data);
            printf(" \r\n Reduce ADC input and press SW3 to start again. \r\n");
        }
        else
        {
            printf("\r\n ADC Data: %ld, Below set DCMP threshold 2047", (long int)*adc_data);
        }
